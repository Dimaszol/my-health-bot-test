...
async def get_user_profile(user_id: int) -> Optional[Dict]:
    ...
    Returns:
        Dict с данными профиля или None
    """
    try:
        conn = await get_db_connection()
        try:
            result = await conn.fetchrow("""
                SELECT name, birth_year, gender, height_cm, weight_kg, 
    ...
    try:
        ...
        try:
            results = await conn.fetch("""
                SELECT message AS message_text, created_at, role AS message_type
                FROM chat_history 
                WHERE user_id = $1 
        ...
        from error_handler import log_error_with_context
        log_error_with_context(e, {"function": "get_recent_messages", "user_id": user_id})
        return []