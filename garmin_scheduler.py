...
class GarminScheduler:
    ...
    def _create_hybrid_record(self, yesterday_data: Optional[Dict], today_data: Optional[Dict], base_date: date) -> Dict:
        ...
        📊 ЧТО ОБЪЕДИНЯЕМ:
        - Активность ВЧЕРА: шаги, пульс, стресс, Body Battery (начало→конец дня)
        - Сон СЕГОДНЯ: длительность, фазы сна (с ночи вчера→сегодня)
        - Восстановление СЕГОДНЯ: Body Battery после сна (утро сегодня)
        
        🔄 ПРИМЕР:
        Анализируем 3 октября утром 4 октября:
        ├─ Активность 3 октября: шаги 14,557, пульс 48, стресс 30
        ├─ Body Battery 3 октября: начало 98% → конец 5%
        ├─ Сон ночью 3→4: 7ч 53м (глубокий 47м, REM 80м)
        └─ Body Battery после сна: 100% (утро 4 октября) ⭐
        """
        ...
        if today_data:
            ...
            # ✨ НОВОЕ: Body Battery после сна (утреннее восстановление)
            # body_battery_max сегодня = максимум утром = результат ночного восстановления
            if today_data.get('body_battery_max') is not None:
                result['body_battery_after_sleep'] = today_data['body_battery_max']
